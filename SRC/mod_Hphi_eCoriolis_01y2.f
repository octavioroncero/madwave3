...
!_________________________________________________!
      ...
      integer :: jcan,jcanp,jelec,jom,jomdi,jomat
      real*8 :: xLam
...
**>> Evaluation of the matrix for j^2
            ...
            do j=1,nojbas(jcan)
               xkinj(i,j)=0.d0
               if(ican.eq.jcan.and.jbas(i,ican).eq.jbas(j,jcan))then
                  xj=dble(jbas(i,ican))
                  xLam=dble(iomdi)  
     ...
     &                 .and. iabs(jomdi-iomdi).eq.1)then
                  ...
                  xom=dsqrt( xj*(xj+1)-dble(iabs(jomdi*iomdi)) )
                  xkinj(i,j)=xLam*xom*hbr*hbr
...
******************************
     ...
     &                                 +xl2p(ir2,iang,jangp,icanp,0))
                     enddo
                  elseif(iom.eq.iom2.and.
     &               iabs(iomdiat(ielec)-iomdiat(ielec2)).eq.1)then
                     ir=1
                     i0=indtotproc(ir,ican2p,iangp,jdproc)-1
                     do ir=1,npunreal(iang)
                        ir1=indr1(ir,iang)
